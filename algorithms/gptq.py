...
The class :class:`GPTQ`:
  ...
  * In :meth:`fasterquant`, performs Cholesky-based block-wise rounding:
    columns are processed left-to-right, and the error from each rounded
    column is propagated to the not-yet-rounded ones using the inverse
    Cholesky factor.
"""
import math
import time
...
class GPTQ:
    ...
    def fasterquant(self, blocksize=128, percdamp=.01, groupsize=-1, actorder=False, name='', alpha=None, args=None):
